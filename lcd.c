...
void lcd_banner_effect(char dispChars[], char position);
int length(char arrayTolong[]);
 ...
 * Microcontrollers are lots of fun!
 * @author Quinten Sorice, Jamie Peterson
 * @date 1/25/19
 * @param char array (string) passed in, prints a string
 */
void lcd_rotatingBanner(char dispText[]){
    ...
    int position = 0;
    while(1) {
        lcd_banner_effect(toDisplay, dispText[position]);
        position++;
        lcd_printf(toDisplay);
        timer_waitMillis(300);
        if(position == length(dispText)) {
            int j;
            for(j = 0; j < LCD_WIDTH; j++){
                lcd_banner_effect(toDisplay, ' ');
                lcd_printf(toDisplay);
                timer_waitMillis(300);
            }
            position = 0;
...
/*
 * Moves the characters to display over one to the right, modifying a String to create a
 * scrolling banner effect.
 * @author Quinten Sorice, Jamie Peterson
 * @date 1/25/19
 * @param char array in, char to change in. modifies the char array.
 */
void lcd_banner_effect(char characters[], char temp)
{
    int i;
    for(i = 0; i < LCD_WIDTH; i++) {
        characters[i] = characters[i + 1];
    }
    characters[LCD_WIDTH-1] = temp;
}

/*
 * Finds the length of a char array because C is dumb and doesn't have this function
 * @author Quinten Sorice, Jamie Peterson
 * @date 1/25/19
 * @param char array in, returns int of array's size
 */
int length(char arrayToInt[]) {
    int length = 0;
    while(arrayToInt[length] != '\0') {
        length++;
    }
    return length;
}
